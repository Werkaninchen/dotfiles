...
update() {
	# Bar
	url=$(jq -r '.weathergov | "\(.url)\(.location)/\(.format)"' $HOME/.config/sketchybar/weather_config.json)
	weather=$(curl -s "$url")
	...
	# popup
	url=$(jq -r '.wttr | "\(.url)\(.location)?\(.format)"' $HOME/.config/sketchybar/weather_config.json)
    echo $url
